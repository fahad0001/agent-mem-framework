...
- `docs/agent-memory/**/index.json` folder indexes
...
Folder indexes are a machine-readable lookup layer for agents and scripts. They
do not replace Markdown artifacts; they help agents find the right Markdown
without scanning the whole memory tree.

1. The root `docs/agent-memory/index.json` must include `folderIndexes.items`.
2. Every non-root folder listed in `folderIndexes.items` must have an `index.json`.
3. Folder indexes are generated by:

```bash
npm run build:memory-indexes
```

4. Check freshness with:

```bash
npm run validate:memory-indexes
```

5. Agents should consult folder indexes first when locating requirement, plan,
   execution, evaluation, decision, audit, template, or metrics artifacts.
6. After adding, removing, or renaming files under `docs/agent-memory/`, rerun
   the builder before final validation.
