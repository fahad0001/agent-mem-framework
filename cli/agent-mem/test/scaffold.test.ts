...
describe("scaffold smoke", () => {
  it("scaffolds a generic project end-to-end (non-interactive)", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "agent-mem-test-"));
    ...
    // agent-mem.config.json written
    expect(fs.existsSync(path.join(targetDir, "agent-mem.config.json"))).toBe(
      true,
    );
  }, 30_000);

  it("emits opencode vendor surface when selected", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "agent-mem-oc-"));
  ...
  it("emits restored framework overlays (KI-0002 closure)", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "agent-mem-overlays-"));
