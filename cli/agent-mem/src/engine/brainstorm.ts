...
export const runBrainstorm = async (
  ...
  p.intro("agent-mem · brainstorm");
  p.note(
    "We'll iteratively shape the project before any code is written.\nAt the end you get project-brief.md (+ .json) feedable to `agent-mem create --from-brief`.",
    "How this works",
  );
...
export const briefToMarkdown = (b: BrainstormBrief): string => {
  ...
  lines.push("");
  lines.push(`_Generated by \`agent-mem brainstorm\` at ${b.generatedAt}._`);
  lines.push("");
