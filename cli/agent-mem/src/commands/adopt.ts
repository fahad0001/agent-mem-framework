...
/**
 * `agent-mem adopt [path]` — brownfield onboarding.
 *
...
export const cmdAdopt = async (opts: AdoptOptions): Promise<void> => {
  const root = opts.cwd;
  log.banner(`agent-mem adopt — ${root}`);
  ...
  const meta = {
    ...
    vendors,
    detection: det,
    generatedBy: "agent-mem adopt",
    generatedAt: new Date().toISOString(),
  };
  fs.writeFileSync(
    path.join(root, "agent-mem.config.json"),
    JSON.stringify(meta, null, 2) + "\n",
    "utf8",
  );
  log.ok("wrote agent-mem.config.json");
  log.info("");
  ...
  log.info("Next: review docs/agent-memory/ then run `agent-mem validate`.");
};
