...
const FORBIDDEN = [
  ...
  { re: /\bas a (large )?language model\b/i, label: "AI self-reference leakage" },
...
if (runCitations) {
  ...
  if (hits === 0)
    ok(`citation scan clean (${cited} citation token${cited === 1 ? "" : "s"} verified)`);
}
...
if (runAhcBlock) {
  ...
  if (!fs.existsSync(dir)) {
    ...
    if (hits === 0)
      ok(`AHC block present in ${agents.length} agent prompt${agents.length === 1 ? "" : "s"}`);
  }
}
